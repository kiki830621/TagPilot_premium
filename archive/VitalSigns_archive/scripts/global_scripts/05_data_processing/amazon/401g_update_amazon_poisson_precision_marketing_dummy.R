...
update_amazon_poisson_precision_marketing_dummy <- function(asin_table, item_property_dictionary) {
  ...
  # 在 item_property_dictionary 中新增 amazon_poisson_precision_marketing 欄位，
  # 若該筆 asin 有出現在 asin_union 中，則設定為 1，否則設定為 0
  item_property_dictionary <- item_property_dictionary %>%
    mutate(amazon_poisson_precision_marketing = if_else(asin %in% asin_union, 1, 0))
  
  return(item_property_dictionary)
}
