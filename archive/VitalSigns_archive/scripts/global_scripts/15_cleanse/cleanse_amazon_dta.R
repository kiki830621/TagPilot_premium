...
#' requiring the item_property_dictionary table. It performs basic data cleaning,
...
cleanse_amazon_dta <- function(raw_data, cleansed_data, verbose = TRUE) {
  ...
  tryCatch({
    ...
    product_cols <- c("sku", "asin", "product_id", "item_id")
    ...
    price_cols <- c("item_price", "price", "unit_price", "total_price")
    ...
    if (has_price) {
      query <- query %>% rename(lineitem_price = !!sym(price_col))
    }
    ...
    if (has_price) {
      query <- query %>% select(customer_id, time, sku, lineitem_price, everything())
    } else {
    ...
    tryCatch({
      ...
      if (has_price && price_col %in% names(raw_data_df)) {
        cleansed_data_df$lineitem_price <- raw_data_df[[price_col]]
      }
    ...
    if (!"lineitem_price" %in% names(cleansed_data_df)) {
      if (verbose) message("Adding default lineitem_price column...")
      cleansed_data_df$lineitem_price <- 1
    }
