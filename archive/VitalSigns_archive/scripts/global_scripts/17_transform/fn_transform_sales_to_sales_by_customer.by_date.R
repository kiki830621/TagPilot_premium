transform_sales_to_sales_by_customer.by_date <- function(df, 
  ...
  if ("lineitem_price" %in% names(df) && !"total_spent" %in% names(df)) {
    if (verbose) message("Field 'lineitem_price' found instead of 'total_spent'. Using 'lineitem_price'.")
    setnames(df, "lineitem_price", "total_spent")
  }
