...
#' @title bs4Dash Conditional Menu Item Example
#' @description Demonstrates conditional menu items based on user input
#' @author Claude
#' @date 2025-04-14
#' @export
bs4dash_conditional_menu_example <- function() {
  ui <- bs4Dash::bs4DashPage(
      ...
      bs4Dash::bs4TabItems(
        bs4Dash::bs4TabItem(
          ...
          bs4Dash::bs4Card(
            ...
            shiny::p("Toggle the checkbox below to show or hide the 'Hidden View' menu item."),
            shiny::checkboxInput("show", "Show Hidden View", FALSE),
            shiny::tags$hr(),
            shiny::p("This example demonstrates how to conditionally display menu items based on user input."),
        ...
        bs4Dash::bs4TabItem(
          ...
          bs4Dash::bs4Card(
            ...
            shiny::p("This page is only accessible when you enable its menu item.")
          )
        )
      )
    ),
    controlbar = bs4Dash::bs4DashControlbar(
      shiny::div(
        class = "p-3",
        "This example demonstrates conditional menu items that appear or disappear based on user input."
      )
    )
  )
  ...
  server <- function(input, output, session) {
    ...
    shiny::observeEvent(input$show, {
      if (input$show) {
        shiny::showNotification("Hidden menu item is now visible", type = "success")
      } else {
        # If the hidden tab is currently selected, switch to tab1 when hiding
        if (!is.null(input$sidebarMenu) && input$sidebarMenu == "tab2") {
          bs4Dash::updatebs4TabItems(session, "sidebarMenu", "tab1")
        }
        shiny::showNotification("Hidden menu item is now invisible", type = "warning")
      }
    })
  }
...
#' @description Demonstrates complex navigation with nested menu items and tab management
#' @author Claude
#' @date 2025-04-14
#' @export
  ...
  shiny::shinyApp(
        ...
        bs4Dash::bs4TabItems(
          ...
          bs4Dash::bs4TabItem(
            ...
            bs4Dash::bs4Card(
              ...
              solidHeader = TRUE,
              width = 12,
              "Content for the first item in group 2."
            )
          ),
          bs4Dash::bs4TabItem(
            tabName = "tab6",
            bs4Dash::bs4Card(
              ...
              solidHeader = TRUE,
              width = 12,
              "Content for the second item in group 2."
            )
          ),
          bs4Dash::bs4TabItem(
            tabName = "tab7",
    ...
    server = function(input, output, session) {
      ...
      output$expandedState <- shiny::renderText({
        expanded <- input$sidebarItemExpanded
        if (is.null(expanded)) {
          "No menu items expanded"
        } else {
          paste("Expanded items:", paste(expanded, collapse = ", "))
        }
      })
