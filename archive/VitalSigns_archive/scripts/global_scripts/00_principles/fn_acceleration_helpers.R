...
scheduleAccelerationMaintenance <- function(session) {
  ...
  if (is_acceleration_enabled("data", "use_caching")) {
    observe({
      ...
      if (cache_size > 0) {
        message("Acceleration cache maintenance: ", cache_size, " cached items")
        
        # If more than 50 items, remove oldest ones
        if (cache_size > 50) {
          # Get list of cached items
          cache_items <- ls(.cache_env)
          # Remove the first 20% of items (oldest)
          to_remove <- cache_items[1:max(1, round(cache_size * 0.2))]
          rm(list = to_remove, envir = .cache_env)
          message("Removed ", length(to_remove), " old cached items")
        }
      }
    })
  }
