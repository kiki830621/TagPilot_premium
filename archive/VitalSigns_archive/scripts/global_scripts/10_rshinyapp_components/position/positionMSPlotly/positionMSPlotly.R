...
#   • Hover information with brand and Item ID details
...
generate_cluster_names <- function(cluster_stats, gpt_key, model = "o4-mini") {
  ...
  cluster_summary <- paste(
    lapply(1:nrow(cluster_stats), function(i) {
      paste0(
        "Cluster ", cluster_stats$cluster[i], ":\n",
        "- Companies: ", cluster_stats$n_companies[i], "\n",
        "- Key characteristics: ", cluster_stats$significant_vars[i], "\n",
        "- Average revenue: $", format(cluster_stats$avg_revenue[i], big.mark = ",")
      )
    }),
    collapse = "\n\n"
  )
  
  # Create prompt based on language configuration
  prompt <- paste0(
    lang_config$ai_naming_instruction, "\n\n",
    paste(
      lapply(1:nrow(cluster_stats), function(i) {
        paste0(
  ...
  tryCatch({
    ...
    message("First 200 chars of prompt: ", substr(prompt, 1, 200), "...")
    
    # Use simplified httr2 syntax
    response <- httr2::request("https://api.openai.com/v1/chat/completions") |>
      httr2::req_auth_bearer_token(gpt_key) |>
      httr2::req_body_json(list(
        model = model,
        messages = list(
          list(role = "system", content = paste(lang_config$system_prompt, "Output R code only: c() with segment names")),
          list(role = "user", content = prompt)
        ),
        max_completion_tokens = 4000  # Increased significantly for o4-mini model
      )) |>
      httr2::req_perform() |>
    ...
    if (length(response$choices) == 0) {
      message("ERROR: No choices in response")
      return(paste("Segment", cluster_stats$cluster))
    }
    ...
    if (is.null(response_text)) {
      message("ERROR: Response content is NULL")
      return(paste("Segment", cluster_stats$cluster))
    }
...
positionMSPlotlyServer <- function(id, app_data_connection = NULL, config = NULL,
  ...
  moduleServer(id, function(input, output, session) {
      ...
      result <- tryCatch({
        ...
        if (!"item_id" %in% names(filtered_data) && nrow(filtered_data) > 0) {
          ...
          if (item_col %in% names(filtered_data)) {
            message("DEBUG: Renaming '", item_col, "' to 'item_id' in positionMSPlotly")
            filtered_data <- filtered_data %>% dplyr::rename(item_id = !!sym(item_col))
          } else {
            warning("No item identifier column found in MS position data. Available columns: ", paste(names(filtered_data), collapse = ", "))
    ...
    output$csa_plot <- renderPlotly({
      ...
      if ("brand" %in% names(data) && "item_id" %in% names(data)) {
    ...
    output$cluster_table <- renderDT({
      ...
      if (!is.null(app_brand_name) && !is.null(csa) && "brand" %in% names(csa)) {
        ...
        if (nrow(app_brand_data) > 0) {
          # Get unique clusters for all Item IDs of this brand
          app_brand_clusters <- unique(as.numeric(app_brand_data$group))
          app_brand_item_id_count <- nrow(app_brand_data)
          
          # Sum revenue across all Item IDs
      ...
      if (!is.null(app_brand_clusters) && length(app_brand_clusters) > 0 && !is.null(app_brand_name)) {
        ...
        # Calculate average revenue per Item ID
        app_brand_avg_revenue <- ifelse(app_brand_item_id_count > 0, 
                                       app_brand_revenue / app_brand_item_id_count, 
                                       0)
        ...
        # Add Item ID count info if multiple Item IDs
        item_id_info <- ifelse(app_brand_item_id_count > 1, 
                           paste0(" - ", app_brand_item_id_count, " Item IDs"), 
                           "")
        
        if (has_revenue) {
          app_brand_footer <- c(
            paste0(app_brand_name, item_id_info),
            "-",
    ...
    # Define chat_api function for market segmentation analysis
    chat_api <- function(msgs, api_key) {
      tryCatch({
        if (is.null(api_key)) {
        ...
        return(paste("Error generating analysis:", e$message))
      })
    }
      ...
      if (component_status() == "ready" && !ms_analysis_generated()) {
        ...
        later::later(function() {
          tryCatch({
            ...
            # Create prompt for market segmentation analysis using CSV table data
            sys <- list(role = "system", content = lang_config$system_prompt)
            usr <- list(
              role = "user", 
              content = paste0(
                lang_config$report_instruction,
                "\n\n## 1. 市場區隔輪廓分析",
                "\n請提供完整的市場區隔輪廓分析報告，例如各群名稱、廠商家數、各群市占率、各群總營收、各群平均每家企業銷售額，以及各群的特色。",
                "\n\n## 2. 目標客群偏好分析", 
                "\n分析產品的目標客群偏好的特色，與其營收和市佔率，在廣告行銷上，須強化產品什麼特色，提供行銷文案建議。",
                "\n\n## 3. 潛在市場機會分析",
                "\n請根據區隔平均營收、區隔總營收、競爭者數量、市場集中度等角度進行分析，提供產品未來可以進入的潛在市場區隔特色，以及行銷策略建議，如需在廣告上強化產品什麼特色，提供行銷文案建議。",
                "\n\n**重要規則**：",
                "\n- 保持 markdown 格式，不要用程式碼區塊包起來",
                "\n- 提供具體的數據洞察和實用建議",
                "\n- 字數限制500字內",
                "\n- 基於下方CSV表格的實際數據進行分析",
                "\n- 表格中的Segment欄位是AI自動命名的市場區隔名稱，請在分析中引用這些名稱",
                "\n- Key Characteristics欄位包含了各區隔的關鍵特徵，請重點分析",
                "\n\n市場區隔表格數據（CSV格式）：",
                "\n```csv",
                "\n", csv_data,
                "\n```"
