...
perform_kfe_analysis <- function(data, exclude_vars = NULL, threshold_multiplier = 1.0) {
  ...
  if (!"item_id" %in% names(data)) {
    ...
    if ("asin" %in% names(data)) {
      ...
      warning("No item identifier column found in KFE analysis data")
      return(list(
  ...
  for (factor in key_factors) {
    if (factor %in% colnames(indicators)) {
      ...
      if (length(factor_indices) > 0) {
        # Get Item IDs for those indices, filtering out NA values
        benchmark_item_ids <- df_no_ideal$item_id[factor_indices]
        benchmark_item_ids <- benchmark_item_ids[!is.na(benchmark_item_ids)]
        benchmark_item_ids <- unique(benchmark_item_ids)
        
        # Only include if we have valid Item IDs
  ...
  if (length(key_factors) > 0) {
    ideal_analysis <- df_no_ideal %>%
      dplyr::select(item_id, brand, dplyr::all_of(key_factors)) %>%
      dplyr::mutate(
      ...
      result <- tryCatch({
        ...
        if (!"item_id" %in% names(filtered_data) && nrow(filtered_data) > 0) {
          ...
          if (item_col %in% names(filtered_data)) {
            message("DEBUG: Renaming '", item_col, "' to 'item_id' in positionKFE")
            filtered_data <- filtered_data %>% dplyr::rename(item_id = !!sym(item_col))
          } else {
            warning("No item identifier column found in KFE position data. Available columns: ", paste(names(filtered_data), collapse = ", "))
          }
        }
