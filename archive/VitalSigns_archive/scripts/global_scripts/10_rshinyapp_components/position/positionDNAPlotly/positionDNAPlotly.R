...
#' @param data data.frame. Position data with brand, item_id, and attribute columns
...
transform_position_to_dna_format <- function(data, exclude_vars = NULL) {
  ...
  dna_data <- data %>%
    tidyr::pivot_longer(
      cols = -c("item_id", "brand"),
      names_to = "attribute",
      values_to = "score"
    ) %>%
    dplyr::arrange(item_id, attribute) %>%
...
create_brand_groups <- function(dna_data) {
  item_id_groups <- split(dna_data, dna_data$item_id)
...
positionDNAPlotlyServer <- function(id, app_data_connection = NULL, config = NULL,
  ...
  moduleServer(id, function(input, output, session) {
      ...
      result <- tryCatch({
        ...
        if (!"item_id" %in% names(filtered_data) && nrow(filtered_data) > 0) {
          ...
          if (item_col %in% names(filtered_data)) {
            message("DEBUG: Renaming '", item_col, "' to 'item_id' in positionDNAPlotly")
            filtered_data <- filtered_data %>% dplyr::rename(item_id = !!sym(item_col))
          } else {
            warning("No item identifier column found in DNA position data. Available columns: ", paste(names(filtered_data), collapse = ", "))
          }
        }
    ...
    output$dna_plot <- renderPlotly({
      ...
      # Create brand groups
      item_id_groups <- create_brand_groups(data)
      ...
      # Add traces for each brand/item_id group
      for (i in seq_along(item_id_groups)) {
        item_id_data <- item_id_groups[[i]]
        brand <- unique(item_id_data$brand)
