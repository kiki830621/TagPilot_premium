...
server <- function(input, output, session) {
  ...
  output$CommonFilter <- renderUI({
    div(
      ...
      radioButtons(
        inputId = "platform",
        label = NULL,
        # Using numeric values as the actual values sent to the server
        choices = list("eBay" = 6, "Cyberbiz" = 7, "All Platforms" = 0),
        selected = 6,  # Make sure this matches your initial default
        inline = FALSE
      )
    )
  })
  ...
  observeEvent(input$platform, {
    ...
    platform_label <- switch(as.character(platform_id),
                         "6" = "eBay",
                         "7" = "Cyberbiz",
                         "0" = "All Platforms",
  ...
  output$customer_display_ui <- shiny::renderUI({
    ...
    if (input$sidebar_menu == "microCustomer") {
      ...
      div(
        ...
        if (debug_mode() && !is.null(input$platform) && length(input$platform) > 0 && as.integer(input$platform) > 0) {
          shiny::div(
  ...
  filtered_customer_data <- shiny::reactive({
    ...
    if (debug_mode()) {
      if (platform_id > 0) {
        message("Server using platform filter ID: ", platform_id)
      } else {
  ...
  shiny::observe({
    shiny::req(input$sidebar_menu, input$platform)
    if (input$sidebar_menu == "microCustomer") {
      # Get the selected platform value (integer)
      selected_platform <- as.integer(input$platform)
      
      # Log the platform filtering (only in debug mode)
      if (selected_platform > 0) {
        if (debug_mode()) {
          message("Using platform filter ID: ", selected_platform)
        }
        
        # Show notification about platform change
        platform_label <- switch(as.character(selected_platform),
                               "6" = "eBay",
                               "7" = "Cyberbiz", 
                               "0" = "All Platforms",
