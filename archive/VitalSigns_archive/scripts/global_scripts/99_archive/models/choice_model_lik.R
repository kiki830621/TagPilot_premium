...
Design_matrix_Generation.choice_model <- function(sub_res,design_list){
  ...
  if(  nrow(dup_sub)>0){
    for (i in dup_sub$Subject_ID) {
      if (i==dup_sub$Subject_ID[1]){
        ...
        colnames(sub_intercept)=paste0(colnames(item_intercept),"_sub_",i)
        
      }else{
        s_inter <- item_intercept
        
        s_idx <- sub_idx_map[Subject_ID==i,idx]
        row_to_keep=sub_res$idx%in%s_idx
        s_inter <- s_inter[, lapply(.SD, function(x) ifelse(row_to_keep, x, 0))]
        
        colnames(s_inter)=paste0(colnames(item_intercept),"_sub_",i)
  ...
  if (is.null(design_list$beta_j.covariate)){
    ...
    for (j in design_list$beta_j.covariate) {
     # j=design_list$beta_j.covariate
      selected_cov <- sub_res[,j,with = FALSE]
      for (i in unique(item_idx_map$Item_ID)) {
        if(i==unique(item_idx_map$Item_ID)[1]){
        
          item_selected_cov <- selected_cov
          
          row_to_keep=sub_res$Item_ID==i
          item_selected_cov <- item_selected_cov[, lapply(.SD, function(x) ifelse(row_to_keep, x, 0))]
          colnames(item_selected_cov)=paste0(colnames(selected_cov),"_item_",i)
        }else{
          s_inter <- selected_cov
          
          row_to_keep=sub_res$Item_ID==i
          s_inter <- s_inter[, lapply(.SD, function(x) ifelse(row_to_keep, x, 0))]
          colnames(s_inter)=paste0(colnames(selected_cov),"_item_",i)
          item_selected_cov=cbind(item_selected_cov,s_inter)
          
        }
      }
      beta_j.mat.small <-  cbind(item_selected_cov)
  ...
  result <- do.call(cbind, Filter(Negate(is.null), list(item_intercept,sub_intercept,beta_i.mat,beta_j.mat)))
  
  columns_to_remove <- grep(paste0("item_",design_list$baseline_item_id,"|sub_",design_list$baseline_id), colnames(result))
  #colnames(result)
  # 删除这些列
  constraint_result <- result[, -columns_to_remove, with = FALSE]
  
  remain_idx_cols <- sub_res[,.( idx,Choice,Subject_ID,Item_ID ,ChoiceSet)]
...
Design_matrix_Generation.choice_model2 <- function(sub_res,design_list){
  ...
  print("start create item intercept")
  S <- Sys.time()
  item_intercept <- do.call(rbind, replicate(idx_num,   diag(1,item_num), simplify = FALSE))
  item_intercept <- item_intercept %>% as.data.table()
  
  colnames(item_intercept) <- paste0("item_",c(1:item_num))
  print("item intercept finished")
  ...
  if(  nrow(dup_sub)>0){
    ...
    for (i in dup_sub$Subject_ID) {
      if (i==dup_sub$Subject_ID[1]){
        sub_intercept = copy(item_intercept)
      ...
      #  colnames(sub_intercept)=paste0(colnames(item_intercept),"_sub_",i)
        ...
        s_inter <-  copy(item_intercept)
        
        s_idx <- sub_idx_map[Subject_ID==i,idx]
        #row_to_keep=sub_res$idx%in%s_idx
        # s_inter[sub_res$idx %in% s_idx, (paste0(colnames(item_intercept), "_sub_", i)) := lapply(.SD, function(x) x)]
        # s_inter[!sub_res$idx %in% s_idx, (paste0(colnames(item_intercept), "_sub_", i)) := 0]
      ...
      #  s_inter[row_to_keep, (paste0(colnames(item_intercept), "_sub_", i)) := lapply(.SD, function(x) ifelse(.I %in% s_idx, x, 0))]
  ...
  if (is.null(design_list$beta_j.covariate)){
    ...
    pb <- progress_bar$new(
      format = "  [:bar] :percent :elapsed",
      total = length(unique(item_idx_map$Item_ID))*length(design_list$beta_j.covariate), clear = FALSE, width = 60
    )
    for (j in design_list$beta_j.covariate) {
      # j=design_list$beta_j.covariate
      selected_cov <- sub_res[,j,with = FALSE]
      for (i in unique(item_idx_map$Item_ID)) {
        if(i==unique(item_idx_map$Item_ID)[1]){
          ...
          item_selected_cov <- copy(selected_cov)
          ...
          columns_to_update=colnames(item_selected_cov)
          item_selected_cov[!sub_res$Item_ID==i, (columns_to_update) := 0]
          
      #    item_selected_cov[!sub_res$Item_ID==i, (colnames(item_selected_cov) ):= 0, .SDcols = colnames(item_selected_cov)]
          
        #  item_selected_cov[sub_res$Item_ID==i, (paste0(colnames(selected_cov), "_item_", i)) := lapply(.SD, function(x) x)]
          #item_selected_cov[!sub_res$Item_ID==i, (paste0(colnames(selected_cov), "_item_", i)) := 0]
          ...
          # colnames(s_inter)=paste0(colnames(selected_cov),"_item_",i)
          # 
          # s_inter[sub_res$Item_ID==i, (paste0(colnames(selected_cov), "_item_", i)) := lapply(.SD, function(x) x)]
          # s_inter[!sub_res$Item_ID==i, (paste0(colnames(selected_cov), "_item_", i)) := 0]
          
          colnames(s_inter)=paste0(colnames(selected_cov),"_item_",i)
          
          columns_to_update=colnames(s_inter)
          s_inter[!sub_res$Item_ID==i, (columns_to_update) := 0]
          
        #  s_inter[!sub_res$Item_ID==i,( colnames(s_inter)) := 0, .SDcols = colnames(s_inter)]
  ...
  # 将剩余的索引列与约束结果合并
  data <- cbind(sub_res[, .(idx, Choice, Subject_ID, Item_ID, ChoiceSet)], constraint_result)
  a <- colnames(data)
  # 转换为稀疏矩阵
...
Design_matrix_Generation.choice_model3 <- function(sub_res,design_list){
    ...
    for (j in design_list$beta_j.covariate) {
      ...
      for (i in unique(item_idx_map$Item_ID)) {
        if(i==unique(item_idx_map$Item_ID)[1]){
          ...
          item_selected_cov <- Matrix(as.matrix(item_selected_cov), sparse = TRUE)
          colnames(item_selected_cov) <- columns_to_update
  ...
  columns_to_update <- colnames(item_intercept)
  item_intercept <- Matrix(as.matrix(item_intercept), sparse = TRUE)
  colnames(item_intercept) <- columns_to_update
  # 假设 `result` 已经是之前操作得到的 data.table
  ...
  sub_sparse <- sub_res[, .(idx, Choice, Subject_ID, Item_ID, ChoiceSet)]
...
choice_llik <- function(data,parameter,lambda=0,alpha=0){
  ...
  item_loc <- which(colnames(data)== "Item_ID")
  c_set_loc <- which(colnames(data)== "ChoiceSet")
  
  # 1. obs 2. choice  3. Subject_ID 4. Item_ID 5. ... 
  
  item_length=length(unique(data[,item_loc]))
  
  item_mat <-data[,-c(idx_loc,sid_loc,c_loc,item_loc,c_set_loc)]
  #item_mat <- item_mat[c(1:item_length),]
  
  obs_num <- length(unique(data[,idx_loc]))
  deno <- item_mat%*%parameter
  c_set <- data[,c_set_loc]
  
  #e_deno <- log(sum(exp(deno)))
  # 如果要有item indicator 加在這邊
  e_deno <- tapply(exp(deno)*c_set, (seq_along(deno) - 1) %/% item_length, sum)
  e_deno <- mean(log(e_deno))
  
  # ch_mat <- data[,-c(1,2,3)]
  ch <- data[,c_loc]
  num <- t(ch) %*%item_mat%*% parameter
...
choice_score <- function(data,parameter,lambda=0,alpha=0){
  ...
  #  e_deno <- log(sum(exp(deno)))
  # e_deno <- mean(log(e_deno))
  e_deno <- tapply(exp(deno)*c_set, (seq_along(deno) - 1) %/% item_length, sum) # new ver
  ...
  e_deno2 <- log(rep(e_deno,each=item_length)) # new ver
  # length(e_deno2) 
  ori_form <- (t(num)-t(t(exp(deno-e_deno2)*c_set)%*%item_mat))/obs_num # new ver
...
predict_acc <- function(data,parameter,lambda=0,alpha=0){
  ...
  pred <- tapply( deno, (seq_along(deno) - 1) %/% item_length, which.is.max)
  act <- tapply(ch, (seq_along(ch) - 1) %/% item_length, which.is.max)
...
predict_acc2 <- function(data,parameter,lambda=0,alpha=0,range=1){
  ...
  result <- tapply(deno, (seq_along(deno) - 1) %/% item_length, get_top_k_positions, k = range)
...
base_line_get <- function(data){
  item_loc <- which(colnames(data)== "Item_ID")
  item_length <- length(unique(data[,item_loc]))
  act <- tapply(data[,which(colnames(data)=="Choice")], (seq_along(data[,which(colnames(data)=="Choice")]) - 1) %/% item_length, which.is.max)
  act_tab <- table(act)
