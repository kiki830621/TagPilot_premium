...
createSidebar <- function(id, active_module) {
  ...
  # Instead of trying to make menuItem work, let's use basic bs4Dash boxes
  ...
  sidebar_items[[length(sidebar_items) + 1]] <- bs4Dash::actionButton(
  ...
  # Return the sidebar items as a div containing all elements
  ...
  sidebar = bs4Dash::dashboardSidebar(
    ...
    conditionalPanel(
      ...
      div(
        class = "sidebar-filters",
        style = "padding: 10px 15px;",
        uiOutput("target_sidebar_items")
      )
    )
  ),
  
  # Main body with tabItems corresponding to menu items
...
server <- function(input, output, session) {
  # Render sidebar items for each module
  output$micro_sidebar_items <- renderUI({
    createSidebar("micro_sidebar", "micro")
  })
  
  output$macro_sidebar_items <- renderUI({
    createSidebar("macro_sidebar", "macro")
  })
  
  output$target_sidebar_items <- renderUI({
