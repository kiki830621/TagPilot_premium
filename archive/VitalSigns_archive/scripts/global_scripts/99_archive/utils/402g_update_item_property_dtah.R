update_item_property_dtah <- function(googlesheet_con,database) {
  
  # 取得此 Google Sheet 中所有工作表的名稱
  all_sheet_names <- sheet_names(googlesheet_con)
  
  # 依序讀取每個工作表，並進行資料處理
  all_item_property_dictionary <- map_dfr(all_sheet_names, function(sheet_nm) {
    # 讀取該工作表的資料
  ...
  dbWriteTable(database, "comment_property_dtah", all_item_property_dictionary, overwrite = TRUE)
...
# write_item_property_dictionary(googlesheet_con)