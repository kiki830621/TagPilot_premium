process_and_join_positions <- function(processed_data, raw_data, app_data, vec_product_line_id_noall) {
  ...
  for (product_line_id in vec_product_line_id_noall) {
    ...
    asin_list <- tbl(raw_data, "item_property_dictionary") %>% 
    ...
    # 從 raw_data 讀取該產品線的 item_property 表（僅選擇 asin 與 brand 欄位）
    item_property <- tbl(raw_data, paste("item_property", product_line_id, sep = "_")) %>% 
      select(asin, brand)
    ...
    # 進行串接：依據 asin 依序 left join 銷售與 item_property 資料，若 brand 為 NA 則填入 "UNKNOWN"
    combined_position <- Dta %>% 
      left_join(cl_used_sales, copy = TRUE, by = join_by(asin)) %>% 
      left_join(item_property, copy = TRUE, by = join_by(asin)) %>% 
