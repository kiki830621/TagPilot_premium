#' Process Comment Property Ratings by Item
#'
#' Processes comment property ratings by item (ASIN for Amazon, ebay_item_number for eBay) for a specific product line. 
...
#' process_comment_property_ratings_by_item(
...
#'
#' @export
process_comment_property_ratings_by_item <- function(processed_data,
                                                   product_line_id,
                                                   impute_missing = TRUE,
                                                   m = 5,
                                                   maxit = 20,
  ...
  new_table <- paste0("df_comment_property_ratingonly_by_", 
                      if (platform == "amz") "asin" else "item", 
                      "_", product_line_id)
  ...
  # Step 1: Export, group by item, and handle NAs
