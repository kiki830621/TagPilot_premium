...
#' 1. Filtering to keep only reviews for competitor items
...
#' 
#' Note: Reviews for items not in the competitor list are removed during processing
#' 
...
#' @param competitor_table Name of competitor table (default: "df_eby_competitor_item_id")
...
process_eby_reviews <- function(cleansed_db_connection = cleansed_data,
                                ...
                                competitor_table = "df_eby_competitor_item_id",
                                overwrite = TRUE) {
  ...
  # 使用 ebay_item_number, reviewer, comment 來判斷是否為重複評論
  # 當有重複時，保留最舊的日期（最早的評論）
  df_eby_review_dedup <- df_eby_review %>%
    dplyr::group_by(ebay_item_number, reviewer, comment) %>%
  ...
  if (competitor_table_exists) {
    # Retrieve competitor items with product line information
    message(paste("Reading competitor data from", competitor_table))
    # First get all competitor items before deduplication
    df_competitor_items_all <- dplyr::tbl(raw_db_connection, competitor_table) %>% 
      dplyr::collect() %>% 
      dplyr::select(
        dplyr::any_of(c("ebay_item_number", "product_line_id", "brand", "seller", "country"))
      ) %>%
      dplyr::mutate(
        # Ensure ebay_item_number is character for joining
        ebay_item_number = as.character(ebay_item_number)
      )
    
    # Find duplicates
    duplicate_items <- df_competitor_items_all %>%
      dplyr::count(ebay_item_number) %>%
      dplyr::filter(n > 1) %>%
      dplyr::arrange(dplyr::desc(n))
    
    if (nrow(duplicate_items) > 0) {
      message(paste("\nFound", nrow(duplicate_items), "duplicate ebay_item_numbers in competitor table:"))
      # Show details of duplicates
      for (i in 1:min(10, nrow(duplicate_items))) {
        item_num <- duplicate_items$ebay_item_number[i]
        count <- duplicate_items$n[i]
        
        # Get details of this duplicate
        dup_details <- df_competitor_items_all %>%
          dplyr::filter(ebay_item_number == item_num) %>%
          dplyr::select(dplyr::any_of(c("ebay_item_number", "product_line_id", "brand", "seller")))
        
        message(paste0("\n  Item: ", item_num, " (", count, " occurrences)"))
        print(dup_details)
      }
      
      if (nrow(duplicate_items) > 10) {
        message(paste("\n  ... and", nrow(duplicate_items) - 10, "more duplicate items"))
      }
      
      message("\nRemoving duplicates - keeping first occurrence of each item number")
    }
    
    # Now deduplicate
    df_competitor_items <- df_competitor_items_all %>%
      dplyr::distinct(ebay_item_number, .keep_all = TRUE)
    
    # Join reviews with competitor data to get product line information
    # Use inner_join to keep only reviews for items in competitor list
    ...
    if (filtered_out > 0) {
      message(paste("Filtered out", filtered_out, 
                    "reviews for non-competitor items"))
      ...
      message("All reviews matched competitor items (no filtering needed)")
    }
    
    # All remaining reviews are for competitor items by definition (inner join)
  ...
  df_eby_review_processed <- df_eby_review_with_info %>%
    ...
    dplyr::relocate(
      dplyr::any_of(c("date", "ebay_item_number", "product_line_id"))
    ) %>%
    # Sort by item number and date
    dplyr::arrange(ebay_item_number, date)
  
  # Show processing statistics
  message("\nProcessing statistics:")
  
  # Total reviews kept (all are competitor items)
  message(paste("Total reviews retained:", nrow(df_eby_review_processed), 
                "(all for competitor items)"))
