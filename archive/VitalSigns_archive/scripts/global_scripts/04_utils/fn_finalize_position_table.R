...
finalize_position_table <- function(app_data) {
  ...
  tryCatch({
    dplyr::tbl(app_data, "df_position") %>% 
      ...
      dplyr::relocate(product_line_id, item_id, brand, rating, sales, 
...
verify_position_table <- function(app_data) {
  ...
  items_count <- DBI::dbGetQuery(
    app_data,
    "SELECT COUNT(DISTINCT item_id) FROM df_position WHERE item_id NOT IN ('Rating', 'Revenue', 'Ideal')"
  )[1, 1]
  
  message("- Product lines: ", product_lines_count)
  message("- Brands: ", brands_count)
  message("- Items: ", items_count)
