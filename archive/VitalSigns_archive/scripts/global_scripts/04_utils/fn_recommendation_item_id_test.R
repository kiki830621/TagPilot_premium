#' Test script for recommend_next_items function
#'
...
#' 2. Source or run this file after fn_recommendation_item_id.R has been loaded
#'
...
if (!exists("recommend_next_items")) {
  if (file.exists("fn_recommendation_item_id.R")) {
    source("fn_recommendation_item_id.R")
  } else {
    stop("Please source fn_recommendation_item_id.R first")
  }
}
...
generate_synthetic_purchase_data <- function(n_customers = 100, 
  ...
  # Generate item IDs and properties
  item_ids <- paste0("P", sprintf("%04d", 1:n_items))
  
  # Create item properties
  item_properties <- data.frame(
    product_id = item_ids,
    category = sample(paste0("Cat", 1:n_categories), n_items, replace = TRUE),
    price = round(runif(n_items, 10, 200), 2),
    rating = round(runif(n_items, 1, 5), 1),
    stock_level = sample(c("Low", "Medium", "High"), n_items, replace = TRUE),
  ...
  for (i in 1:n_customers) {
    ...
    preferred_items <- item_properties %>%
    ...
    preferred_items$preference_score <- preferred_items$preference_score + rnorm(n_items, 0, 0.5)
    ...
    purchased_items <- character(n_purchases)
    
    for (j in 1:n_purchases) {
      if (j == 1) {
        # First purchase - choose from preferred items with higher probability
        purchased_items[j] <- sample(
          preferred_items$product_id, 
          1, 
          prob = exp(preferred_items$preference_score) / sum(exp(preferred_items$preference_score))
        )
      } else {
        # Subsequent purchases - influence by previous purchase
        prev_item <- purchased_items[j-1]
        prev_category <- item_properties$category[item_properties$product_id == prev_item]
        
        # Increase probability for items in same category
        category_boost <- ifelse(item_properties$category == prev_category, 1.5, 1)
        
        # Avoid immediate duplicates but allow later repeats
        prev_items <- purchased_items[1:(j-1)]
        repeat_penalty <- ifelse(item_properties$product_id %in% prev_items, 0.5, 1)
        
        # Combine factors
        adjusted_scores <- preferred_items$preference_score * category_boost * repeat_penalty
  ...
  # Join with item properties
  purchase_data <- all_purchases %>%
    left_join(item_properties, by = "product_id")
...
# Run recommendations
recommended_items <- recommend_next_items(
  df = purchase_data,
  customer_id_col = "customer_id",
  item_id_col = "product_id",
  time_col = "purchase_date",
  covariate_cols = selected_covariates,
  recommended_number = 5,  # Recommend 5 items for simplicity
  seed = 456,
...
cat("Recommendations generated for", nrow(recommended_items), "customers\n\n")
print(head(recommended_items, 10))

# Analyze recommendation diversity
cat("\nAnalyzing recommendation diversity...\n")

# Reshape recommendations for analysis
recommendation_long <- recommended_items %>%
  tidyr::pivot_longer(
    cols = starts_with("recommended_next_item_id"),
    names_to = "rank",
    values_to = "product_id"
  )

# Count frequency of recommended items
item_frequency <- recommendation_long %>%
  group_by(product_id) %>%
  summarize(count = n()) %>%
  arrange(desc(count))
...
print(head(item_frequency, 10))

# Calculate how many unique items were recommended
unique_recommended <- length(unique(recommendation_long$product_id))
total_possible <- length(unique(purchase_data$product_id))
cat("\nRecommendation coverage:", 
    unique_recommended, "unique items recommended out of", 
    total_possible, "possible items (", 
    round(100 * unique_recommended / total_possible, 1), "%)\n")

# Create a visualization of recommendation distribution
if (requireNamespace("ggplot2", quietly = TRUE)) {
  # Top 20 items by recommendation frequency
  top_items <- head(item_frequency, 20)
  
  # Create plot
  p <- ggplot(top_items, aes(x = reorder(product_id, count), y = count)) +
    geom_bar(stat = "identity", fill = "steelblue") +
    coord_flip() +
    labs(title = "Top 20 Most Frequently Recommended Items",
         x = "Product ID",
...
# Are recommended items in the same category as previously purchased items?

# Get the last purchased item for each customer
