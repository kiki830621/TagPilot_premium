...
process_amz_reviews <- function(cleansed_db_connection = cleansed_data,
  ...
  product_line_df_norm <- product_line_df %>% 
    dplyr::mutate(cat_name = janitor::make_clean_names(product_line_name_english, ascii = FALSE))
  ...
  df_amz_review <- dplyr::tbl(cleansed_db_connection, source_table) %>% 
    ...
    dplyr::mutate(
      ...
      cat_name = raw_cat %>% 
        ...
        janitor::make_clean_names(allow_dupes = TRUE, ascii = FALSE)
    ) %>% 
  ...
  # Retrieve competitor items with flag
  df_competitor_item_id_with_flag <- dplyr::tbl(raw_data, "df_competiter_item_id") %>% 
    dplyr::collect() %>% 
    dplyr::mutate(included_competiter = TRUE)
  
  # Add competitor flag to reviews
  df_amz_review_with_flags <- df_amz_review %>% 
    dplyr::left_join(df_competitor_item_id_with_flag) %>% 
