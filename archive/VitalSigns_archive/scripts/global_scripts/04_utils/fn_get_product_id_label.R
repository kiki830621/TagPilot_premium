...
#' This follows R123 (Unified Item ID Rule) for display purposes while maintaining
...
#' 
#' # Get label for eBay
#' get_product_id_label("eby")  # Returns "Item Number"
#' 
#' # Get label for all platforms
#' get_product_id_label("all")  # Returns "Item ID"
#' }
#'
#' @export
get_product_id_label <- function(platform_id, df_platform = NULL, translate = identity) {
  ...
  # Default label
  default_label <- translate("Item ID")
  ...
  label_map <- c(
    "asin" = translate("ASIN"),
    "ebay_item_number" = translate("Item Number"),
    "sku" = translate("SKU"),
