...
#' @param comment_sample_size Integer. Number of comments to sample per item for analysis (default: 10)
...
import_comment_property_ratings <- function(raw_data = raw_data,
  ...
  # Get item ID column name from df_platform
  ...
  item_id_col <- platform_info$product_id_coding
  
  if (is.na(item_id_col)) {
  ...
  for (product_line_id_i in vec_product_line_id_noall) {
    ...
    new_columns <- janitor::make_clean_names(df_comment_property___filtered$property_name,
                                             ascii = FALSE)
    ...
    df_review___new_columns <- df_review___filtered %>% 
      arrange(!!sym(item_id_col), date) %>%       # Sort by item ID and date
      group_by(!!sym(item_id_col)) %>%            # Group by item ID
      mutate(
    ...
    # Create sampled data for competitor items (for manual review or NLP processing)
    # Using the configurable comment_sample_size parameter
    # Note: For eBay, all items in processed data are competitors by definition
    df_review___sampled <- df_review___new_columns %>%
      slice_tail(n = comment_sample_size) %>% 
      ungroup()
    
    # Log sample size
    message("Sampled competitors: ", paste(names(table(df_review___sampled[[item_id_col]])), 
                                       collapse = ", "))
