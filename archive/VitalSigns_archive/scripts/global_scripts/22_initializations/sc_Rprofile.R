...
.InitEnv$autoinit <- function() {
  ...
  if (!exists("APP_DIR", envir = .InitEnv)) {
    ...
    list2env(list(
      ...
      GLOBAL_DIR           = file.path(base, "update_scripts", "global_scripts"),
      GLOBAL_DATA_DIR      = file.path(base, "update_scripts", "global_scripts", "global_data"),
      GLOBAL_PARAMETER_DIR = file.path(base, "update_scripts", "global_scripts", "global_data", "parameters"),
      CONFIG_PATH          = file.path(base, "app_config.yaml"),
      APP_DATA_DIR         = file.path(base, "app_data"),
      APP_PARAMETER_DIR    = file.path(base, "app_data", "parameters")
    ), envir = .InitEnv)
  }
