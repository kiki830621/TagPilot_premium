...
#
# USAGE: Copy this template and replace {platform} with actual platform code
# ==============================================================================
...
test_execution <- function() {
  tryCatch({
    ...
    # Test 2: Verify platform code consistency across all tables
    ...
    for (table_name in expected_tables) {
      if (DBI::dbExistsTable(raw_data, table_name)) {
        ...
        if (length(platform_codes) != 1 || platform_codes[1] != "{platform}") {
          stop(sprintf("Platform code inconsistent in %s. Expected: {platform}, Found: %s", 
                       table_name, paste(platform_codes, collapse = ", ")))
        }
      }
    }
    
    message("TEST: ✅ Platform codes consistent across all tables")
