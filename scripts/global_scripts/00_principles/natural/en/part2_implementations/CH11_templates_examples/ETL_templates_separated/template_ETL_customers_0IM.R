...
#
# USAGE: Copy this template and replace {platform} with actual platform code (cbz, eby, amz, etc.)
# ==============================================================================
...
test_execution <- function() {
  tryCatch({
    ...
    # Test 4: Verify platform code consistency
    message("TEST: 🏷️  Verifying platform code consistency...")
    ...
    if (length(platform_codes) != 1 || platform_codes[1] != "{platform}") {
      stop(sprintf("Platform code inconsistent. Expected: {platform}, Found: %s", 
                   paste(platform_codes, collapse = ", ")))
    }
    
    message(sprintf("TEST: ✅ Platform code consistent: %s", platform_codes[1]))
...
# 
# 1. Replace all instances of "{platform}" with the actual 3-letter platform code:
