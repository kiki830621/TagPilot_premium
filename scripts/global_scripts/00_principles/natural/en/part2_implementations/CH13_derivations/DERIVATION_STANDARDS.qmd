...
**Components**:
- `{platform}`: Three-letter platform code (cbz, amz, eby) or 'all'
...
| **PLATFORM** | Platform code or 'all' | "cbz" |
...
#' @param platform_filter Optional platform code to process (default: all)
...
get_platform_info <- function(script_name = NULL) {
  ...
  if (!platform_code %in% names(VALID_PLATFORMS)) {
    stop(sprintf("Invalid platform code: %s", platform_code))
  }
