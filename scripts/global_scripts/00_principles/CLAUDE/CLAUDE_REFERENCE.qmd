...
## 🚨 PRE-TASK CHECKLIST (MUST READ EVERY TIME)

### 1. Working Directory Setup
- [ ] **ALWAYS** change to project root: `/Users/che/Library/CloudStorage/Dropbox/che_workspace/projects/ai_martech/l4_enterprise/WISER`
...
Rscript scripts/update_scripts/amz_ETL04_0IM.R  # ❌ autoinit() not found
```

#### Key Environment Files
- `.Rprofile` → Sources `scripts/global_scripts/22_initializations/sc_Rprofile.R`
- `sc_Rprofile.R` → Defines `autoinit()` function in `.InitEnv`
- `autoinit()` → Automatically detects script location and sets `OPERATION_MODE`
...
Rscript script_name.R  # ❌ WILL FAIL
```
...
scripts/
├── global_scripts/
│   ├── 00_principles/         # Principles and guidelines
│   ├── 05_etl_utils/         # ETL utility functions
│   └── 04_utils/             # General utility functions
└── update_scripts/           # Main execution scripts
```
...
# Verify data flow: raw → staged → transformed
```
...
### 3. Database Schema
```
raw_data.duckdb       → staged_data.duckdb       → transformed_data.duckdb
├── df_*              ├── df_*___staged          ├── df_*___transformed
└── Import phase      └── Staging phase         └── Transform phase
```
