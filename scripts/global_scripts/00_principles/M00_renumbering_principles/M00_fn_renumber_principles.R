...
#'
#' @description
#' This is the main entry point for the M00_renumbering_principles module.
...
M00_renumbering_principles <- new.env()
...
for (file in function_files) {
  source(file, local = M00_renumbering_principles)
}
...
#'
M00_renumbering_principles$rename <- function(old_name, new_name, update_refs = TRUE) {
  # Call the actual implementation
  M00_renumbering_principles$rename_resource(old_name, new_name, update_refs)
}
...
#' @return A list with success status and details
#'
M00_renumbering_principles$renumber <- function(old_id, new_id, name) {
  # Call the actual implementation
  M00_renumbering_principles$renumber_resource(old_id, new_id, name)
}
...
#'
M00_renumbering_principles$batch_renumber <- function(mapping_table) {
  # Call the actual implementation
  M00_renumbering_principles$batch_renumber_resources(mapping_table)
}
...
#'
M00_renumbering_principles$verify <- function() {
  # Call the actual implementation
  M00_renumbering_principles$verify_consistency()
}
