...
# Shows KPI value boxes for high-level sales metrics
...
macroOverviewServer <- function(id, data_source) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      # Placeholder count values for customer segments
      output$N_nes_count_now <- renderText({ "1,200" })
      output$N_nes_count_diff_value <- renderText({ "+150" })
      output$E0_nes_count_now <- renderText({ "1,000" })
      output$E0_nes_count_diff_value <- renderText({ "+75" })
      output$S1_nes_count_now <- renderText({ "800" })
      output$S1_nes_count_diff_value <- renderText({ "-20" })
      output$S2_nes_count_now <- renderText({ "400" })
      output$S2_nes_count_diff_value <- renderText({ "-35" })
      output$S3_nes_count_now <- renderText({ "600" })
      output$S3_nes_count_diff_value <- renderText({ "-45" })
    })
  })
}