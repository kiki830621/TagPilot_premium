...
#'
#' This function loads data from the shared global_data directory, which contains
...
load_from_global_data <- function(file_name, version = "current", required = TRUE, default = NULL, write_mode = FALSE) {
  ...
  base_path <- file.path("update_scripts", "global_scripts", "global_data")
  ...
  if (!check_data_access("global", access_type, path)) {
    stop(access_type, " access to global_data not allowed in current operating mode")
  }
  ...
  if (interactive()) {
    message("EXCEPTION NOTE: Accessing shared global_data resource: ", file_name)
  }
