...
test_chat_api <- function() {
  cat("=== Testing fn_chat_api function ===\n\n")
  ...
  # Test 1: Default model (gpt-4o-mini)
  cat("\n--- Test 1: Default model (gpt-4o-mini) ---\n")
  tryCatch({
    result1 <- fn_chat_api(messages, api_key)
  ...
  # Test 2: gpt-4o-mini explicitly
  cat("\n--- Test 2: gpt-4o-mini explicitly ---\n")
  tryCatch({
    result2 <- fn_chat_api(messages, api_key, model = "gpt-4o-mini")
    cat("✅ gpt-4o-mini test passed\n")
    cat("Response:", substr(result2, 1, 100), "...\n")
  }, error = function(e) {
    cat("❌ gpt-4o-mini test failed:", e$message, "\n")
  })
  
  # Test 3: o4-mini-deep-research
  cat("\n--- Test 3: o4-mini-deep-research ---\n")
  tryCatch({
    result3 <- fn_chat_api(messages, api_key, model = "o4-mini-deep-research")
  ...
  cat("\n--- Test 4: o1-mini ---\n")
  tryCatch({
    result4 <- fn_chat_api(messages, api_key, model = "o1-mini")
  ...
  tryCatch({
    result5 <- fn_chat_api(messages, api_key, model = "invalid-model-name")
...
test_parameter_methods <- function() {
  ...
  cat("\n--- Method 1: Named parameter ---\n")
  tryCatch({
    result1 <- fn_chat_api(messages, api_key, model = "gpt-4o-mini")
  ...
  cat("\n--- Method 2: Positional parameter ---\n")
  tryCatch({
    result2 <- fn_chat_api(messages, api_key, "gpt-4o-mini")
...
check_model_availability <- function() {
  ...
  for (model in models_to_test) {
    ...
    tryCatch({
      result <- fn_chat_api(simple_message, api_key, model = model)
      cat("✅ Available\n")
    }, error = function(e) {
...
run_all_tests <- function() {
  cat("Starting comprehensive fn_chat_api tests...\n")
  cat("Time:", Sys.time(), "\n")
