...
mamba_aggregate_sales <- function(con, start_date, end_date) {
  query <- glue::glue("
    ...
    FROM daily_sales ds
    JOIN platform_totals pt USING (platform_code)
...
mamba_inventory_optimization <- function(con, lookback_days = 30) {
  query <- glue::glue("
    WITH sales_velocity AS (
      ...
      GROUP BY product_id, platform_code
    ),
    inventory_levels AS (
      SELECT 
        product_id,
    ...
    LEFT JOIN sales_velocity sv USING (product_id, platform_code)
...
mamba_cross_platform_compare <- function(con, metric = "revenue", period = "month") {
  ...
  query <- glue::glue("
    ...
    SELECT 
      ...
      LAG(metric_value) OVER (PARTITION BY platform_code ORDER BY period_date) as prev_value,
      ((metric_value / NULLIF(LAG(metric_value) OVER (PARTITION BY platform_code ORDER BY period_date), 0)) - 1) * 100 as growth_pct
    FROM platform_ranks
...
mamba_load_platform_config <- function(con) {
  ...
  default_configs <- data.frame(
    platform_code = c("AMZ", "EBY", "SHP", "LAZ"),
  ...
  for (i in 1:nrow(default_configs)) {
    ...
    ", default_configs$platform_code[i],
