...
ui <- bs4DashPage(
  ...
  sidebar = bs4DashSidebar(status="primary", width="300px", elevation = 3,
                           ...
                           div(class="sidebar-section p-3 mt-2",
                               ...
                               radioButtons("platform", NULL, c("eBay"=6,"Cyberbiz"=7,"All"=0), 6)),
                           # 動態 filter 容器
...
server <- function(input, output, session){
  # ---- 6.1 共享設定 ------------------------------------------------------
  comp_config <- reactive({ list(filters = list(platform_id = as.integer(input$platform))) })
  ...
  observeEvent(input$platform, {
    lbl <- switch(as.character(input$platform), "6"="eBay", "7"="Cyberbiz", "0"="All Platforms")
