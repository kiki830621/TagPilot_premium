...
microCustomerServer <- function(id, 
  ...
  moduleServer(id, function(input, output, session) {
    ...
    # Extract platform_id from config or input
    platform_id <- reactive({
      cfg <- if (is.reactive(config)) config() else config
      pid <- tryCatch({
        if (!is.null(cfg) && !is.null(cfg[["filters"]])) {
          cfg[["filters"]][["platform_id"]]
        } else {
          NULL
        }
      }, error = function(e) NULL)
      
      if (is.null(pid) && !is.null(input[["platform"]])) pid <- input[["platform"]]
      as.integer(pid %||% 0L)
    })
    ...
    observeEvent(df_prof(), {
      ...
      # Handle case with no valid customer IDs
      if (length(ids) == 0 || nrow2(prof) == 0) {
        # SLN07: Always use a descriptive text label for empty options
        # to avoid "attempt to use zero-length variable name" error
      ...
      # Create filtered customer choices
      choices <- prof %>%
      ...
      # Handle case where filter produced no results
      if (nrow2(choices) == 0) {
        updateSelectizeInput(session, "customer_filter", 
                            choices = c("No matching customers" = ""),
                            selected = "")
        return()
      }
      
      # Create named list for choices
      choice_list <- setNames(as.character(choices[["customer_id"]]), choices[["label"]])
      
      # SLN07: Use descriptive text for placeholder option to avoid zero-length variable name error
      updateSelectizeInput(session, "customer_filter", 
