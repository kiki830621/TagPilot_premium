  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$avg_purchase_cycle <- renderUI({
      ...
      if (!is.null(values$zscore_metadata$mu_ind)) {
        h2(round(values$zscore_metadata$mu_ind, 1), style = "margin: 0;")
      } else {
        # Use ipt (inter-purchase time) instead of avg_ipt
        avg_ipt <- mean(values$dna_results$data_by_customer$ipt, na.rm = TRUE)
        h2(round(avg_ipt, 1), style = "margin: 0;")
      }
    })
